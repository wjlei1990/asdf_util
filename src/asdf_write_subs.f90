...
!! 3)write_asdf_file
  ...
  integer, parameter :: nparam_head=30
  character(len=200), dimension(2, nparam_head) :: head_list
  DATA head_list / &
    "i", "npoints", &
    "i", "gmt_year", &
    "i", "gmt_day", &
...
subroutine define_asdf_data (adios_group, my_group_size, asdf_container, rank, nproc, comm, ierr)
  ...
  !nrecords info
  call define_asdf_scalar (adios_group, my_group_size, "", "nreceivers", dum_int)
  call define_asdf_scalar (adios_group, my_group_size, "", "nrecords", dum_int)
  !frequency(period) info
  call define_asdf_scalar (adios_group, my_group_size, "", "min_period", dum_double)
  call define_asdf_scalar (adios_group, my_group_size, "", "max_period", dum_double)

  !string info
  call define_asdf_scalar (adios_group, my_group_size, "", "receiver_name_len", &
                        dum_int)
  call define_asdf_scalar (adios_group, my_group_size, "", "network_len", &
                        dum_int)
  call define_asdf_scalar (adios_group, my_group_size, "", "receiver_id_len", &
                        dum_int)
  call define_asdf_scalar (adios_group, my_group_size, "", "component_len", &
                        dum_int)

  !RESPONSE FLAG: store as integer cause logical is not supported 
  call define_asdf_scalar (adios_group, my_group_size, "", "STORE_RESPONSE", &
                        dum_int)

  do i=1, nparam_head
    data_type=head_list(1,i)
    header=head_list(2,i)
    select case (data_type(1:1))
      case ("i")
        call define_asdf_global_integer_1d_array (adios_group, my_group_size,&
                    nrecords, "", trim(header), int_array)
      case("r")
        call define_asdf_global_real_1d_array (adios_group, my_group_size, &
                    nrecords, "", trim(header), real_array)
      case ("d")
        call define_asdf_global_double_1d_array (adios_group, my_group_size, &
      ...
      case ("s")
        ...
        call define_asdf_local_string_1d_array (adios_group, my_group_size,&
    ...
    do i = 1, nrecords
      write(i_string, '(I10)' ) i+offset
                record="RESP."//trim(asdf_container%network_array(i))//"."//&
                                               ...
                                               trim(asdf_container%receiver_id_array(i))//"."//&
                                               trim(asdf_container%component_array(i))
      call define_asdf_global_byte_1d_array (adios_group, my_group_size,&
...
subroutine write_asdf_file_sub (asdf_container, adios_handle, rank, nproc, comm, ierr)
  ...
  do i = 1, asdf_container%nrecords
    ...
    loc_string=trim(asdf_container%receiver_name_array(i))//"."//&
               ...
               trim(asdf_container%component_array(i))//"."//&
               trim(asdf_container%receiver_id_array(i))
    call write_asdf_global_double_1d_array(adios_handle, rank, nproc, &
  ...
  if (asdf_container%STORE_RESPONSE) then
    ...
    do i = 1, asdf_container%nrecords
      ...
      loc_string="RESP."//trim(asdf_container%network_array(i))//"."//&
               ...
               trim(asdf_container%receiver_id_array(i))//"."//&
               trim(asdf_container%component_array(i))
      call write_asdf_global_byte_1d_array(adios_handle, rank, nproc, &
  ...
  call write_asdf_global_double_1d_array(adios_handle, rank, nproc, asdf_container%nrecords, &
